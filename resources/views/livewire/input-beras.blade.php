    ...
    <form wire:submit.prevent="store" class="max-w-7xl mx-auto">
        ...
        <div class="bg-[#1a1d2d] border border-gray-700/50 rounded-2xl p-6 shadow-xl mb-6 relative overflow-hidden group">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
                ...
                <div>
                    <label class="block text-gray-400 text-xs font-bold mb-2">ID MO / Nomor MO <span class="text-red-500">*</span></label>
                    ...
                    @error('id_mo') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-4 relative z-10">
                ...
                <div>
                    <label class="block text-gray-400 text-xs font-bold mb-2">Tanggal Pemeriksaan <span class="text-red-500">*</span></label>
        ...
        <div class="bg-[#1a1d2d] border border-gray-700/50 rounded-2xl p-6 shadow-xl mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="text" wire:model="rendemen_pengolahan" readonly class="w-full bg-[#11131f] border border-gray-700 text-orange-400 font-bold rounded-lg px-4 py-3 cursor-not-allowed text-right">
                </div>
            </div>
            ...
            <div class="bg-[#11131f] p-4 rounded-xl border border-gray-800 grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <input type="number" step="0.01" wire:model="hasil_samping_menir" class="w-full bg-gray-900 border border-gray-700 text-white text-sm rounded px-3 py-2 focus:border-orange-500">
                </div>
                <div>
                    <label class="block text-gray-500 text-[10px] font-bold mb-1">Butir Patah (Kg)</label>
                    <input type="number" step="0.01" wire:model="hasil_samping_butir_patah" class="w-full bg-gray-900 border border-gray-700 text-white text-sm rounded px-3 py-2 focus:border-orange-500">
                </div>
                <div>
                    <label class="block text-gray-500 text-[10px] font-bold mb-1">Dedak/Katul (Kg)</label>
                    <input type="number" step="0.01" wire:model="hasil_samping_dedak_katul" class="w-full bg-gray-900 border border-gray-700 text-white text-sm rounded px-3 py-2 focus:border-orange-500">
                </div>
                <div>
                    <label class="block text-gray-500 text-[10px] font-bold mb-1">Butir Kuning/Rusak (Kg)</label>
                    <input type="number" step="0.01" wire:model="hasil_samping_butir_kuning_rusak" class="w-full bg-gray-900 border border-gray-700 text-white text-sm rounded px-3 py-2 focus:border-orange-500">
                </div>
            </div>
        </div>
        ...
        <div class="bg-[#1a1d2d] border border-gray-700/50 rounded-2xl p-6 shadow-xl mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4 relative z-10">
                ...
                <div>
                    ...
                    <input type="text" wire:model="status" readonly class="w-full bg-[#11131f] border border-gray-700 text-green-500 font-bold rounded-lg px-4 py-3 cursor-not-allowed">
                </div>
                <div>
        ...
        <div class="bg-[#1a1d2d] border border-gray-700/50 rounded-2xl p-6 shadow-xl mb-6 relative overflow-hidden">
            <div class="flex items-center justify-between mb-2">
                ...
                <button type="button" wire:click="addFotoRow"
                    class="bg-emerald-600/20 hover:bg-emerald-600 text-emerald-300 hover:text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-colors">
                    + Tambah Foto
                </button>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div wire:key="foto-row-{{ $key }}" class="bg-[#11131f] border border-gray-800 rounded-xl p-4 flex flex-col sm:flex-row items-start gap-3">
                    <label for="foto-{{ $key }}"
                        class="flex-shrink-0 w-28 h-28 border-2 border-dashed border-gray-700 rounded-lg cursor-pointer bg-gray-800 hover:border-emerald-500 transition-colors flex items-center justify-center overflow-hidden relative">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <input type="text" wire:model.defer="fotoNama.{{ $key }}" placeholder="Contoh: Sampling / Karung"
                            class="w-full bg-gray-800 border border-gray-700 text-white text-sm rounded-lg px-3 py-2 focus:border-emerald-500 focus:outline-none">
                        <div wire:loading wire:target="fotos.{{ $key }}" class="text-emerald-400 text-[11px] mt-1 font-bold animate-pulse">Memproses gambar...</div>
        ...
        <div class="flex flex-col sm:flex-row justify-end gap-4 mt-8 pt-6 border-t border-gray-800">
            ...
            <button type="submit" class="w-full sm:w-auto px-8 py-3 rounded-xl bg-gradient-to-r from-green-600 to-emerald-500 hover:from-green-500 hover:to-emerald-400 text-white font-bold shadow-lg shadow-green-500/30 transition-all transform hover:scale-105">
                Create Data
            </button>
        </div>
    </form>

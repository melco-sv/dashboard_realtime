<div class="min-h-screen bg-[#0b0c15] p-6 text-white font-['Space_Grotesk']">
    ...
    <div class="max-w-5xl mx-auto mb-6">
        <div class="flex justify-between items-start">
            <div>
                <h1 class="text-3xl font-bold text-white">Galeri Foto Beras</h1>
                <p class="text-gray-400 text-sm mt-1">
                    <span class="font-mono text-green-400">{{ $beras->nomor_hpkk_beras }}</span>
                    &bull; {{ $beras->tempat_pemeriksaan ?? '-' }}
                    &bull; {{ $beras->tanggal_pemeriksaan ? $beras->tanggal_pemeriksaan->format('d M Y') : '-' }}
                </p>
            </div>
        ...
        <div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3 bg-gray-900 border border-gray-800 rounded-xl p-4">
            <div>
                ...
                <p class="text-sm text-white uppercase">{{ $beras->lokasi ?? '-' }}</p>
            </div>
            <div>
                <p class="text-[10px] text-gray-500 uppercase tracking-wider font-bold mb-0.5">ID MO</p>
                <p class="text-sm font-mono text-gray-300">{{ $beras->id_mo ?? '-' }}</p>
            </div>
            <div>
                <p class="text-[10px] text-gray-500 uppercase tracking-wider font-bold mb-0.5">Kuantum Beras</p>
                <p class="text-sm font-bold text-green-400">
                    {{ $beras->kuantum_beras ? number_format($beras->kuantum_beras, 2, ',', '.') . ' Kg' : '-' }}
                </p>
            </div>
            <div>
                <p class="text-[10px] text-gray-500 uppercase tracking-wider font-bold mb-0.5">Jumlah Foto</p>
                <p class="text-sm font-bold text-green-400">{{ $fotos->count() }} foto</p>
            </div>
        </div>
    ...
    <div class="max-w-5xl mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div class="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden hover:border-green-800/50 transition-colors">
